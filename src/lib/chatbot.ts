...
async function saveChatbotRecord(input: { message: string; reply: string; history: ChatMessage[]; meta?: ChatbotRecordMeta }) {
  try {
...
export async function askChatbotClient({ message, history = [], chatUiMode = "natural", selectedTreePath = [] }: AskChatbotInput): Promise<{ reply: string; error: string | null }> {
  ...
  try {
    ...
    const qa = rows.find((row) => {
      ...
      return answer && (question.includes(q) || q.includes(question) || title.includes(q) || q.includes(title));
    });

    const asksDrySkin = q.includes("dry") || q.includes("dehydrat") || q.includes("hydration") || q.includes("moisture");
    const asksGlowyBase = q.includes("glow") || q.includes("glowy") || q.includes("dewy") || q.includes("radiant");
    const asksCushion = q.includes("cushion") || q.includes("base makeup") || q.includes("foundation");

    if (qa?.answer) {
      reply = String(qa.answer);
      meta = { ...meta, matched_documents: [], matched_chunks: [], confidence: 0.9, needs_review: false };
    ...
    } else if ((q.includes("trois") || q.includes("touch") || q.includes("troistouch")) && asksCushion && asksGlowyBase) {
      ...
      const product = rankMatches(
        ...
        rows
          ...
          .map((row) => ({ title: row.title, content: row.content ?? "", tags: row.tags ?? [] })),
        1,
      )[0];

      if (product) {
        reply = `Here's what I found: ${product.row.title ?? "Product information"}\n\n${product.row.content || "Please contact our team for details."}`;
        meta = { ...meta, matched_documents: [], matched_chunks: [], confidence: Math.min(product.score, 1), needs_review: product.score < 0.35 };
        ...
        meta = {
          ...meta,
          matched_documents: Array.from(new Map(documentMatches.map((match) => [match.document_id, { id: match.document_id, title: match.document_title, category: match.category }])).values()),
          matched_chunks: documentMatches.map((match) => ({ id: match.id, document_id: match.document_id, chunk_index: match.chunk_index, score: match.score })),
          confidence: Math.min(best.score, 1),
          needs_review: best.score < 0.35,
        };
      } else if (q.includes("b2b") || q.includes("dealer") || q.includes("distributor") || q.includes("wholesale") || q.includes("partner")) {
        reply = "For B2B partnership, wholesale, dealership, or distribution inquiries, please submit the B2B form. Our GPCLUB Vietnam team will review and contact you directly.";
        meta = { ...meta, matched_documents: [], matched_chunks: [], confidence: 0.5, needs_review: false };
      } else if (q.includes("jmsolution") || q.includes("jm solution")) {
        reply = "JMsolution is one of GPCLUB's key K-Beauty brands, known for skincare and mask-pack lines. Tell me your skin type or concern and I can guide you to suitable products.";
        meta = { ...meta, matched_documents: [], matched_chunks: [], confidence: 0.5, needs_review: false };
      } else if (q.includes("jmella") || q.includes("j mella")) {
        reply = "Jmella focuses on fragrance-inspired body and lifestyle care. If you share your preferred scent or product type, I can suggest a direction.";
        meta = { ...meta, matched_documents: [], matched_chunks: [], confidence: 0.5, needs_review: false };
      }
    }
  } catch (caught) {
