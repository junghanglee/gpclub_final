...
export const DEFAULT_BRAND_DETAILS: Record<string, BrandDetail> = {
	jmsolution: {
		...
		sub: "Skin Science",
		tagline:
			"Professional K-beauty skincare built for hydration, recovery, and visible radiance.",
		story:
	...
	jmella: {
		...
		tagline:
			"French-inspired fragrance care for body, hair, and daily self-care rituals.",
		story:
...
function mergeDetail(base: BrandDetail, extra: unknown): BrandDetail {
	...
	const str = (
		k: "key" | "name" | "sub" | "tagline" | "story" | "heritage" | "hero",
...
export function mergeBrandDetails(
	stored: unknown,
	...
	for (const key of Object.keys(DEFAULT_BRAND_DETAILS)) {
		if (isObj(src[key]))
			out[key] = mergeDetail(DEFAULT_BRAND_DETAILS[key], src[key]);
	}
	// Also accept brand keys that only exist in stored data (admin-added brands).
	for (const key of Object.keys(src)) {
		if (!out[key] && isObj(src[key]))
			out[key] = mergeDetail(EMPTY_DETAIL, src[key]);
	}
	return out;
}

export async function fetchBrandDetails(): Promise<
	Record<string, BrandDetail>
> {
...
export function useBrandDetails() {
	const [details, setDetails] = useState<Record<string, BrandDetail>>(
