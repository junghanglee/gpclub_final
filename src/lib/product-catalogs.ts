...
function normalizeCatalog(value: Partial<ProductCatalog>): ProductCatalog {
	...
	return {
		...
		subtitle: String(
			value.subtitle || "Curated K-Beauty portfolio for B2B partners",
		),
		description: String(value.description || ""),
		template:
			value.template === "compact" || value.template === "lineup"
				? value.template
				: "premium",
		product_ids: Array.isArray(value.product_ids)
			? value.product_ids.map(String)
			: [],
...
export async function saveProductCatalogs(catalogs: ProductCatalog[]) {
	const normalized = catalogs.map((catalog, index) => ({
		...
		is_representative:
			...
			catalogs.findIndex((item) => item.is_representative) === index,
	}));
...
export async function fetchRepresentativeCatalog() {
	...
	return (
		catalogs.find((catalog) => catalog.is_representative) || catalogs[0] || null
	);
}
...
export function useRepresentativeCatalog() {
	...
	const downloadPath = useMemo(
		() => (catalog ? `/catalog/${catalog.id}` : "/catalog"),
		[catalog],
	);
