...
import { Sparkles, Sparkle, Package, Briefcase, MessageCircle, ArrowRight } from "lucide-react";
...
const SUGGESTIONS: { icon: React.ReactNode; title: { vi: string; en: string }; sub: { vi: string; en: string } }[] = [
  {
    icon: <Sparkle className="h-4 w-4" />,
    title: { vi: "Danh mục bán lại", en: "Reseller Lineup" },
    sub: { vi: "Gợi ý danh mục JMsolution / Jmella / Trois Touch theo kênh bán", en: "JMsolution / Jmella / Trois Touch lineup guidance by sales channel" },
  },
  {
    icon: <Package className="h-4 w-4" />,
    title: { vi: "Câu chuyện bán hàng", en: "Sales Story" },
    sub: { vi: "Thông điệp bán hàng, điểm mạnh sản phẩm và cách trưng bày", en: "Sales messages, product strengths and merchandising angles" },
  },
  {
    icon: <Briefcase className="h-4 w-4" />,
    title: { vi: "Hợp tác B2B", en: "B2B Partnership" },
    sub: { vi: "Kết nối đại lý, phân phối và yêu cầu bán sỉ", en: "Dealer, distribution and wholesale inquiry connection" },
  },
  {
    icon: <MessageCircle className="h-4 w-4" />,
    title: { vi: "Hỗ trợ đối tác", en: "Partner Support" },
    sub: { vi: "Kết nối báo giá, tài liệu sản phẩm và thông tin hợp tác", en: "Quote, product material and partnership support routing" },
  },
];
...
function GippyAIPage() {
  ...
  return (
    <main className="min-h-[calc(100vh-5rem)] bg-gradient-to-b from-background via-background to-secondary/30">
      ...
      <section id="gippy-guide" className="border-t border-border/60 bg-background/90 py-10 md:py-14">
        <div className="mx-auto max-w-[1100px] px-4 sm:px-6 lg:px-10">
          <div className="mb-6 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
            <div>
              <div className="text-[11px] font-bold uppercase tracking-[0.24em] text-primary">Gippy Quick Guide</div>
            ...
            <p className="max-w-xl text-sm font-medium leading-relaxed text-muted-foreground">
              {lang === "vi" ? "Trang được tinh gọn để tải nhanh hơn và kết nối trực tiếp đến kênh liên hệ." : "For faster loading, this page is now simplified into a lightweight guide and contact flow."}
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {SUGGESTIONS.map((item) => (
              <Link
                key={item.title.en}
                to="/contact"
                search={{ topic: item.title.en } as any}
              ...
              >
                ...
                <h3 className="font-display text-base font-bold text-foreground">{pick(item.title)}</h3>
                <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{pick(item.sub)}</p>
                <div className="mt-4 inline-flex items-center text-xs font-bold uppercase tracking-[0.16em] text-primary">
                  {lang === "vi" ? "Liên hệ" : "Contact"} <ArrowRight className="ml-1 h-3.5 w-3.5" />
                </div>
              </Link>
            ))}
          </div>
...
function GippyHeroSection({ page }: { page: ReturnType<typeof usePageContent> }) {
  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-border/60 bg-gradient-luxe">
      <div aria-hidden className="pointer-events-none absolute -top-36 right-[-12%] h-[520px] w-[520px] rounded-full bg-primary/10 blur-3xl" />
      <div aria-hidden className="pointer-events-none absolute -bottom-44 -left-24 h-[420px] w-[420px] rounded-full bg-accent/60 blur-3xl" />
      <div className="relative mx-auto grid w-full max-w-[1200px] grid-cols-1 items-center gap-8 px-4 py-12 sm:px-6 md:py-16 lg:grid-cols-12 lg:px-10">
        <div className="text-center lg:col-span-7 lg:text-left">
          ...
          <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center lg:justify-start">
            <Button asChild size="lg" className="group h-12 rounded-none bg-foreground px-7 text-sm font-bold uppercase tracking-[0.16em] text-background hover:bg-primary">
              <Link to="/contact">{page.primaryCta[lang]} <ArrowRight className="ml-2 h-4 w-4 transition group-hover:translate-x-1" /></Link>
            </Button>
            <Button asChild size="lg" variant="outline" className="h-12 rounded-none border-foreground px-7 text-sm font-bold uppercase tracking-[0.16em] text-foreground hover:border-primary hover:bg-primary hover:text-primary-foreground">
          ...
          <div className="mt-8 grid grid-cols-3 gap-2 text-left sm:max-w-xl">
            {[
              ...
              <div key={top} className="rounded-2xl border border-border/70 bg-background/70 p-3 shadow-sm backdrop-blur">
                <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">{top}</div>
        ...
        <div className="relative flex justify-center lg:col-span-5 lg:justify-end">
          <div className="relative aspect-[4/5] w-full max-w-[342px] sm:max-w-[342px]">
            <div aria-hidden className="absolute left-[10%] top-[8%] h-10 w-10 rounded-full bg-primary/20 blur-sm" />
            <img
              src={gippyAiHero}
