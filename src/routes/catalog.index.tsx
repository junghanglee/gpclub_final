...
export const Route = createFileRoute("/catalog/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Search, preview and download GPCLUB Vietnam product catalogs." },
    ],
  }),
...
function CatalogDownloadPage() {
  ...
  const filteredCatalogs = useMemo(() => {
    ...
    return catalogs.filter((catalog) => {
      const productNames = catalog.product_ids
        ...
        .filter(Boolean)
        .map((product) => `${product?.brand_name} ${product?.product_name} ${product?.product_type}`)
        .join(" ");
      return `${catalog.title} ${catalog.subtitle} ${catalog.description} ${productNames}`.toLowerCase().includes(query);
    });
  ...
  useEffect(() => {
    ...
    if (!filteredCatalogs.some((catalog) => catalog.id === activeCatalogId)) {
      setActiveCatalogId(filteredCatalogs.find((catalog) => catalog.is_representative)?.id || filteredCatalogs[0].id);
    }
  ...
  const copy = lang === "vi" ? {
    ...
    subtitle: "Chọn catalog ở phía trên, xem nhanh nội dung sản phẩm và tải bản PDF riêng cho từng catalog.",
    ...
    subtitle: "Choose a catalog from the top tabs, review the included products, then download the exact PDF you need.",
    ...
    <main className="min-h-screen bg-gradient-luxe">
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-10">
        <div className="max-w-3xl">
          ...
          <p className="mt-5 text-base font-semibold leading-7 text-foreground/72 md:text-lg">{copy.subtitle}</p>
        </div>
        ...
        <div className="mt-10 flex items-center gap-3 rounded-2xl border border-border bg-white/80 p-3 shadow-soft backdrop-blur">
          ...
          <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={copy.search} className="border-0 bg-transparent shadow-none focus-visible:ring-0" />
        </div>
        ...
        <div className="mt-8">
          ...
          {filteredCatalogs.length > 0 ? (
            <Tabs value={activeCatalogId || filteredCatalogs[0].id} onValueChange={setActiveCatalogId}>
              <TabsList className="sticky top-20 z-20 flex h-auto w-full flex-wrap justify-start gap-2 rounded-3xl border border-border bg-white/90 p-2 shadow-soft backdrop-blur">
                {filteredCatalogs.map((catalog) => (
                  <TabsTrigger key={catalog.id} value={catalog.id} className="rounded-2xl px-4 py-3 text-left data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                    <span className="flex flex-col items-start gap-1">
                      <span className="flex items-center gap-1.5 text-sm font-black">
                        {catalog.is_representative ? <Star className="h-3.5 w-3.5" /> : null}
                        {catalog.title}
                      </span>
                      <span className="text-[11px] font-semibold opacity-80">{catalog.product_ids.length} {copy.products}</span>
                    </span>
                  </TabsTrigger>
                ))}
              </TabsList>
              ...
              {filteredCatalogs.map((catalog) => {
                const selectedProducts = catalog.product_ids.map((id) => productMap.get(id)).filter(Boolean) as CatalogProduct[];
                const brands = Array.from(new Set(selectedProducts.map((product) => product.brand_name).filter(Boolean)));
                return (
                  <TabsContent key={catalog.id} value={catalog.id} className="mt-6">
                    <article className="rounded-[2rem] border border-border bg-white/90 p-5 shadow-soft backdrop-blur md:p-7">
                      <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            {catalog.is_representative ? <Badge className="gap-1"><Star className="h-3 w-3" /> {copy.representative}</Badge> : null}
                            <Badge variant="outline">{copy.template}: {catalog.template}</Badge>
                            <Badge variant="secondary">{catalog.product_ids.length} {copy.products}</Badge>
                          </div>
                          <h2 className="mt-4 font-display text-3xl font-black leading-tight md:text-4xl">{catalog.title}</h2>
                          <p className="mt-2 text-base font-semibold text-muted-foreground">{catalog.subtitle}</p>
                          {catalog.description ? <p className="mt-3 max-w-3xl text-sm leading-6 text-foreground/70">{catalog.description}</p> : null}
                          {brands.length ? <p className="mt-4 line-clamp-2 text-xs font-bold uppercase tracking-[0.16em] text-primary">{brands.join(" · ")}</p> : null}
                        </div>
                        <div className="flex shrink-0 flex-col gap-2 sm:flex-row md:flex-col">
                          <Button asChild className="rounded-none bg-foreground text-background hover:bg-primary">
                            <Link to="/catalog/$catalogId" params={{ catalogId: catalog.id }} target="_blank">
                      ...
                      <div className="mt-8">
                        <h3 className="text-sm font-black uppercase tracking-[0.2em] text-muted-foreground">{copy.includedProducts}</h3>
                        <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                          {selectedProducts.slice(0, 24).map((product) => (
                            <div key={product.id} className="rounded-2xl border border-border/70 bg-background/70 p-4">
                              <div className="text-[11px] font-black uppercase tracking-[0.18em] text-primary">{product.brand_name}</div>
                              <div className="mt-1 font-semibold leading-snug">{product.product_name}</div>
                              <div className="mt-1 text-xs text-muted-foreground">{product.product_type}</div>
                            </div>
                          ))}
                        </div>
                      </div>
