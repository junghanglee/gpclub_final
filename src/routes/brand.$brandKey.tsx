...
export const Route = createFileRoute("/brand/$brandKey")({
	loader: async ({ params }) => {
		const details = await fetchBrandDetails().catch(
			() => DEFAULT_BRAND_DETAILS,
		);
		const data =
			details[params.brandKey] ?? DEFAULT_BRAND_DETAILS[params.brandKey];
			...
			<section className="border-t border-border bg-background py-20 md:py-28">
				<div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-10">
					...
					<div className="mt-8 flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.22em] text-foreground/55">
						<CheckCircle2 className="h-4 w-4 text-primary" /> Verified by GPCLUB
						Vietnam R&D
					</div>
				</div>
			</section>
							...
							return (
								...
								>
									<div>
										...
										<p className="mt-2 max-w-md text-[13px] text-foreground/65">
											{b.tagline}
										</p>
									</div>
