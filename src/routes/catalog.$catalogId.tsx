    ...
    return (
      <div className="catalog-section grid gap-3 px-8 py-10 print:grid-cols-2 print:gap-2 print:px-0 print:py-0">
        {products.map((product, index) => (
          <CompactProductRow key={product.id} product={product} index={index} />
        ))}
      </div>
    );
  }
  if (catalog.template === "lineup") {
    return (
      <div className="catalog-section grid gap-4 px-8 py-10 sm:grid-cols-2 lg:grid-cols-3 print:grid-cols-3 print:gap-3 print:px-0 print:py-0">
        {products.map((product, index) => (
          <LineupProductCard key={product.id} product={product} index={index} />
        ))}
      </div>
    );
  }
...
function ProductCard({ product, index }: { product: CatalogProduct; index: number }) {
  ...
  const visibleConditions = product.conditions.filter(
    (condition) => condition.visible && condition.value,
  );
...
function CompactProductRow({ product, index }: { product: CatalogProduct; index: number }) {
  const image = getCoverImage(product);
  const visibleConditions = product.conditions
    .filter((condition) => condition.visible && condition.value)
    .slice(0, 2);
  ...
  return (
    <article className="break-inside-avoid grid grid-cols-[76px_1fr] gap-3 rounded-xl border border-slate-200 bg-white p-3 print:grid-cols-[52px_1fr] print:gap-2 print:p-2">
      ...
      <div className="min-w-0">
        ...
        <h2 className="mt-1 line-clamp-1 font-display text-base font-black leading-tight print:text-sm">
          {product.product_name}
        </h2>
        <p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-600 print:line-clamp-1 print:text-[10px]">
          {product.short_intro}
        </p>
        {visibleConditions.length > 0 ? (
  ...
  return (
    <article className="break-inside-avoid overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm print:rounded-xl print:shadow-none">
      <div className="relative aspect-square overflow-hidden bg-[#f6efe9]">
        ...
        <div className="absolute left-3 top-3 rounded-full bg-white/90 px-2 py-1 text-[9px] font-black text-pink-600">
          {String(index + 1).padStart(2, "0")}
        </div>
      </div>
      <div className="p-4 print:p-3">
        <div className="text-[10px] font-black uppercase tracking-[0.18em] text-pink-600">
          {product.brand_name}
        </div>
        <h2 className="mt-2 line-clamp-2 font-display text-lg font-black leading-tight print:text-sm">
          {product.product_name}
        </h2>
        <div className="mt-2 text-xs font-semibold text-slate-500 print:text-[10px]">
          {product.product_type}
        </div>
      </div>
    </article>
