...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Marketing and B2B partnership hub for proven K-Beauty brands in Vietnam." },
    ],
  }),
  component: HomePage,
});
...
const partnerBenefits: { title: BilingualText; text: BilingualText }[] = [
  {
    ...
    text: { vi: "Nguồn hàng trực tiếp giúp tối ưu giá nhập và hạn chế rủi ro trung gian.", en: "Direct official supply helps protect pricing, margin and channel transparency." },
  },
  {
    title: { vi: "Danh mục dễ bán", en: "Shelf-ready portfolio" },
    text: { vi: "Nước hoa, chăm sóc da, tóc và body care phù hợp nhiều phân khúc khách hàng.", en: "Skincare, fragrance, body care and makeup designed for multiple reseller and retail channels." },
  },
  {
    title: { vi: "Marketing cùng triển khai", en: "Co-marketing support" },
    text: { vi: "Hỗ trợ nội dung, đào tạo sản phẩm và chiến dịch địa phương hóa cho đối tác.", en: "Localized content, product training and launch assets to help move inventory." },
  },
];

const partnerProcess: { step: string; title: BilingualText; text: BilingualText }[] = [
  { step: "01", title: { vi: "Chia sẻ mục tiêu", en: "Share your goal" }, text: { vi: "Kênh bán, khu vực, sản lượng dự kiến hoặc nhu cầu OEM/ODM.", en: "Tell us your channel, region, volume target or OEM/ODM requirement." } },
  { step: "02", title: { vi: "Nhận đề xuất", en: "Get a proposal" }, text: { vi: "Đội ngũ GPCLUB tư vấn danh mục, giá và kế hoạch ra mắt phù hợp.", en: "We recommend portfolio, pricing structure and go-to-market plan." } },
  { step: "03", title: { vi: "Ra mắt và tăng trưởng", en: "Launch and scale" }, text: { vi: "Đồng hành với tài liệu bán hàng, chiến dịch và kế hoạch bổ sung hàng.", en: "We support sales materials, campaigns and replenishment planning." } },
];
...
function HomePage() {
  ...
  const homeProducts = catalogProducts.filter((p) => p.is_featured || p.is_new || p.is_popular).slice(0, 3);
  const hero = {
  ...
  return (
    <>
      <section ref={heroRef} className="relative isolate overflow-hidden bg-gradient-luxe">
        <div aria-hidden className="pointer-events-none absolute -top-40 right-[-10%] h-[600px] w-[600px] rounded-full bg-primary/10 blur-3xl" />
        <div aria-hidden className="pointer-events-none absolute -bottom-40 -left-20 h-[420px] w-[420px] rounded-full bg-accent/60 blur-3xl" />
        ...
        <div className="relative mx-auto grid min-h-[calc(100svh-4rem)] w-full max-w-[1400px] grid-cols-1 items-center gap-8 px-4 py-12 sm:px-6 sm:py-16 lg:min-h-[92vh] lg:grid-cols-12 lg:gap-10 lg:px-10">
          ...
          >
            ...
            <div className="mt-8 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center lg:items-start lg:justify-start">
              <Button type="button" size="lg" onClick={() => setInquiryOpen(true)} className="group h-12 w-full justify-center rounded-none bg-foreground px-5 text-xs font-bold uppercase tracking-[0.14em] text-background hover:bg-primary sm:w-auto sm:px-7 sm:text-sm sm:tracking-[0.18em]">
                {hero.primaryCta} <ArrowRight className="ml-2 h-4 w-4 transition group-hover:translate-x-1" />
              </Button>
              <Button asChild size="lg" variant="outline" className="group h-12 w-full justify-center rounded-none border-foreground px-5 text-xs font-bold uppercase tracking-[0.14em] text-foreground hover:border-primary hover:bg-primary hover:text-primary-foreground sm:w-auto sm:px-7 sm:text-sm sm:tracking-[0.18em]">
                <Link to={downloadPath} target={downloadPath.startsWith("/catalog/") ? "_blank" : undefined}>
          ...
          >
            <div className="relative mx-auto aspect-[4/5] w-full max-w-[300px] sm:max-w-[380px] lg:ml-auto lg:mr-0 lg:max-w-[421px]">
              <PinkStar className="absolute left-[6%] top-[14%] h-10 w-10 rotate-[12deg]" delay={0} />
              <PinkStar className="absolute right-[12%] top-[6%] h-16 w-16 -rotate-12" delay={0.4} />
              <PinkStar className="absolute right-[4%] top-[40%] h-8 w-8 rotate-[24deg]" delay={0.8} />
              <motion.img
      ...
      <section className="border-t border-border bg-background py-16 md:py-28">
        <div className="mx-auto grid max-w-[1200px] gap-10 px-4 sm:px-6 md:grid-cols-12 lg:px-10">
          <div className="md:col-span-5">
            <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{homeContent.partnerHook.kicker[lang]}</div>
            <h2 className="mt-4 font-display text-3xl font-black leading-[1.1] tracking-tight text-foreground sm:text-4xl md:text-5xl">
              {homeContent.partnerHook.title[lang].replace(homeContent.partnerHook.highlight[lang], "")}<span className="bg-gradient-pink bg-clip-text text-transparent">{homeContent.partnerHook.highlight[lang]}</span>
            </h2>
      ...
      <section className="border-y border-border">
        <div className="mx-auto grid max-w-[1600px] grid-cols-1 md:grid-cols-3">
          {brands.map((b, idx) => {
            const copy = brandCopy[b.name]?.[lang] ?? { tagline: b.tagline, description: b.description };
            return (
              <Link
                key={b.name}
                to="/brand"
              ...
              >
                <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-foreground/60 sm:text-[11px] sm:tracking-[0.3em]">{copy.tagline}</div>
                <div className="mt-3 font-display text-4xl font-black tracking-tight sm:text-5xl md:text-7xl">{b.name}</div>
                <p className="mt-5 max-w-md text-sm leading-relaxed text-foreground/70">{copy.description}</p>
                <span className="mt-7 inline-flex items-center gap-1 text-sm font-bold uppercase tracking-widest text-primary">
                  {lang === "vi" ? "Tìm hiểu" : "Discover"} <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" />
                </span>
              </Link>
            );
          })}
      ...
      <section className="bg-secondary py-16 md:py-32">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-10">
          <div className="max-w-2xl">
            <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{homeContent.trust.kicker[lang]}</div>
          ...
          <div className="mt-10 grid gap-px overflow-hidden border border-border bg-border md:mt-14 md:grid-cols-3">
            <Pillar num="01" icon={<ShieldCheck className="h-5 w-5" />} eng={homeContent.pillars[0].eng[lang]} title={homeContent.pillars[0].title[lang]} text={homeContent.pillars[0].text[lang]} />
            <Pillar num="02" icon={<FlaskConical className="h-5 w-5" />} eng={homeContent.pillars[1].eng[lang]} title={homeContent.pillars[1].title[lang]} text={homeContent.pillars[1].text[lang]} />
            <Pillar num="03" icon={<Globe2 className="h-5 w-5" />} eng={homeContent.pillars[2].eng[lang]} title={homeContent.pillars[2].title[lang]} text={homeContent.pillars[2].text[lang]} />
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-10">
          <div className="grid gap-10 lg:grid-cols-12">
            <div className="lg:col-span-5">
              <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{homeContent.process.kicker[lang]}</div>
              <h2 className="mt-4 font-display text-3xl font-black leading-tight tracking-tight md:text-5xl">
            ...
            <div className="grid gap-px overflow-hidden border border-border bg-border lg:col-span-7">
              {partnerProcess.map((p) => (
                <div key={p.step} className="grid gap-4 bg-background p-6 sm:grid-cols-[80px_1fr] sm:p-8">
                  ...
                  <div>
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-foreground/70">{p.text[lang]}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-10">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div className="max-w-xl">
              <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{homeContent.images.kicker[lang]}</div>
              <h2 className="mt-4 font-display text-3xl font-black leading-tight tracking-tight md:text-5xl">
            ...
            <Link to="/products" className="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-[0.22em] text-foreground transition hover:text-primary">
          ...
          <div className="mt-10 grid gap-4 md:mt-14 md:grid-cols-3">
            {(homeProducts.length > 0 ? homeProducts : []).map((product) => (
              <Link key={product.id} to="/products/$productId" params={{ productId: product.id }} className="group overflow-hidden border border-border bg-card transition hover:-translate-y-1 hover:shadow-soft">
                {getCoverImage(product) ? <img src={getCoverImage(product)} alt={product.product_name} className="aspect-[4/3] w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" /> : <ImagePlaceholder label={product.product_name} />}
                <div className="p-5">
                  <div className="text-[10px] font-bold uppercase tracking-[0.22em] text-primary">{product.brand_name}</div>
                  <h3 className="mt-2 font-display text-xl font-black">{product.product_name}</h3>
                  <p className="mt-2 line-clamp-2 text-sm text-foreground/65">{product.short_intro}</p>
                </div>
              </Link>
            ))}
            {homeProducts.length === 0 && homeContent.images.labels[lang].map((label, index) => (
              <ImagePlaceholder key={`${label}-${index}`} label={label} src={homeContent.images.urls[index]} alt={homeContent.images.alts[lang][index] || label} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="relative isolate overflow-hidden bg-foreground py-16 text-background md:py-24">
        <div aria-hidden className="pointer-events-none absolute inset-0 bg-gradient-to-tr from-primary/20 via-transparent to-primary/10" />
        <div className="relative mx-auto max-w-[1200px] px-4 text-center sm:px-6 lg:px-10">
          <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{homeContent.cta.kicker[lang]}</div>
          ...
          <Button type="button" size="lg" onClick={() => setInquiryOpen(true)} className="mt-10 h-12 w-full justify-center rounded-none bg-primary px-6 text-xs font-bold uppercase tracking-[0.14em] text-primary-foreground hover:bg-primary/90 sm:w-auto sm:px-8 sm:text-sm sm:tracking-[0.18em]">
...
function ImagePlaceholder({ label, src, alt }: { label: string; src?: string; alt?: string }) {
  return (
    <div className="group relative flex aspect-[4/3] min-h-[180px] flex-col items-center justify-center overflow-hidden border border-dashed border-primary/40 bg-primary/5 p-6 text-center sm:p-8">
      {src ? (
        <>
          <img src={src} alt={alt || label} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" />
          <div className="absolute inset-0 bg-gradient-to-t from-foreground/60 via-foreground/10 to-transparent" />
          <div className="relative z-10 text-[11px] font-bold uppercase tracking-[0.28em] text-background drop-shadow">{label}</div>
        </>
      ) : (
        <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary sm:text-[11px] sm:tracking-[0.28em]">{label}</div>
      )}
    </div>
  );
}
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div className="min-w-0 text-center sm:text-left">
      ...
      <div className="mt-1 break-words text-[9px] font-bold uppercase leading-snug tracking-[0.12em] text-muted-foreground sm:text-[10px] sm:tracking-[0.22em]">{label}</div>
    </div>
  );
}

function Pillar({ num, icon, eng, title, text }: { num: string; icon: React.ReactNode; eng: string; title: string; text: string }) {
  return (
    <div className="group relative flex flex-col gap-5 bg-background p-6 transition hover:bg-accent/40 sm:p-10">
      <div className="flex items-start justify-between">
        <div className="font-display text-5xl font-black text-foreground/10 transition group-hover:text-primary/30">{num}</div>
