...
type Card = { num: string; icon: LucideIcon; hook: LocalText; title: string; copy: LocalText };
type Value = { num: string; icon: LucideIcon; eng: string; title: LocalText; sub: LocalText; copy: LocalText };
type BrandSellingPoint = { name: string; fallbackImage: string; copy: LocalText };
...
const WHY_CARDS: Card[] = [
  {
    num: "01",
    icon: PackageCheck,
    hook: tx("Danh mục có lực bán thật: JMsolution + Jmella.", "A sales-ready portfolio: JMsolution + Jmella."),
  ...
  },
  {
    num: "02",
    icon: ShieldCheck,
    hook: tx("Khoa học Hàn Quốc. Cảm xúc Pháp. Phù hợp Việt Nam.", "Korean science. French emotion. Fit for Vietnam."),
  ...
  },
  {
    num: "03",
    icon: Megaphone,
    hook: tx("Bạn bán hàng. GPVN hỗ trợ tăng tốc chuyển đổi.", "You sell. GPVN helps accelerate conversion."),
...
const VALUES: Value[] = [
  {
    ...
    title: tx("Danh mục làm đẹp được thiết kế để chuyển đổi.", "A beauty portfolio built to convert."),
  ...
  },
  {
    ...
    title: tx("Chính hãng, minh bạch và an toàn cho đối tác.", "Official, transparent, and partner-safe."),
  ...
  },
  {
    ...
    title: tx("Từ ý tưởng đến kệ hàng với kinh nghiệm GPCLUB.", "From concept to shelf with GPCLUB know-how."),
...
const IMAGE_SLOTS = ["Partner store / shelf image", "Campaign visual / KOL content", "Training or product demo image"];
...
const b2bText = {
  vi: {
    ...
    heroDesc: "Trở thành đối tác của GPCLUB Vietnam để khai thác danh mục JMsolution và Jmella — hai thương hiệu kết hợp khoa học chăm sóc da Hàn Quốc, hương thơm Pháp và chiến lược bản địa hóa cho thị trường Việt Nam.",
    ...
    processDesc: "Chúng tôi cùng đối tác xác định kênh, danh mục, thông điệp và kế hoạch triển khai phù hợp.",
    process: ["Đánh giá mức độ phù hợp kinh doanh", "Đề xuất danh mục và kênh bán", "Đào tạo, tài sản thương hiệu và kế hoạch ra mắt", "Triển khai chiến dịch và hỗ trợ bán hàng"],
    imagesKicker: "Không gian hình ảnh",
    imagesTitle: "Không gian hình ảnh giúp tăng sức thuyết phục B2B.",
    imagesDesc: "Các khung ảnh được bố trí cân bằng cho hình ảnh kệ bán hàng, chiến dịch và đào tạo sản phẩm.",
    slots: ["Hình ảnh cửa hàng / kệ đối tác", "Visual chiến dịch / nội dung KOL", "Hình ảnh đào tạo hoặc demo sản phẩm"],
    replace: "Thay bằng hình ảnh chính thức",
    contactKicker: "Liên hệ",
    contactTitle: "Bắt đầu cuộc trao đổi.",
    contactDesc: "Chia sẻ mục tiêu kinh doanh, đội ngũ hợp tác của chúng tôi sẽ phản hồi trong 24 giờ.",
    step: "Bước",
    doneTitle: "Đã gửi yêu cầu",
    doneDesc: "Tư vấn viên hợp tác chuyên trách sẽ liên hệ trong 24 giờ với đề xuất kinh doanh phù hợp.",
    backHome: "Về trang chủ",
    qPurpose: "Bạn đang quan tâm điều gì?",
    qPurposeDesc: "Có thể chọn nhiều mục.",
    purposes: ["Phân phối chính hãng / mua sỉ số lượng lớn", "Đại lý độc quyền địa phương / mở flagship store", "Yêu cầu sản xuất OEM / ODM riêng", "Hợp tác marketing và đề xuất kinh doanh khác"],
    ...
    namePh: "Tên của bạn",
    email: "Địa chỉ email",
    consent: "Tôi đồng ý cho thu thập và sử dụng thông tin cá nhân (tên, email, công ty) để xem xét đề xuất kinh doanh.",
    back: "Quay lại",
    continue: "Tiếp tục",
    sending: "Đang gửi…",
  ...
  en: {
    ...
    heroDesc: "Partner with GPCLUB Vietnam to grow with JMsolution and Jmella — brands combining Korean skin science, French fragrance, and localized strategy for the Vietnamese market.",
    ...
    brandsTitle: "A portfolio designed for reseller demand, wholesale channels and retail storytelling.",
    ...
    process: ["Business fit review", "Portfolio & channel proposal", "Training, assets & launch plan", "Campaign execution & sell-through support"],
    imagesKicker: "Image Spaces",
    imagesTitle: "Image spaces that strengthen B2B persuasion.",
    imagesDesc: "Prepared as balanced image spaces for retail, campaign, and product training images supplied later.",
    slots: ["Partner store / shelf image", "Campaign visual / KOL content", "Training or product demo image"],
    ...
    doneDesc: "Our dedicated partnership consultant will reach out within 24 hours with a tailored business proposal.",
    backHome: "Back to Home",
    qPurpose: "What brings you here?",
    qPurposeDesc: "Select all that apply.",
    purposes: ["Official distribution / wholesale bulk purchase", "Exclusive local agency / flagship store opening", "Custom OEM / ODM manufacturing request", "Marketing collaboration & other business proposals"],
    ...
    namePh: "Your name",
    email: "Email address",
    consent: "I consent to the collection and use of my personal information (name, email, company) for business proposal review.",
    back: "Back",
    continue: "Continue",
    sending: "Sending…",
...
function B2BPage() {
  ...
  const sampleProductImages = useMemo(() => {
    ...
    for (const brandName of preferredBrands) {
      const product = catalogRows.find((row) => row.brand_name === brandName && getCoverImage(row) && !used.has(getCoverImage(row)));
      if (product) {
  ...
  const fieldsByStep: (keyof FormData)[][] = [["purposes"], ["company"], ["name", "email", "consent"]];
  ...
  const submit = form.handleSubmit(async (data) => {
    ...
    try {
      ...
      toast.error(t.toastFail, { description: e?.message ?? t.toastFailDesc });
    } finally {
      setSubmitting(false);
    }
  });
  ...
  if (done) {
    return (
      <section className="mx-auto max-w-2xl px-4 py-32 text-center sm:px-6">
        <motion.div initial={{ scale: 1, rotate: -90 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: "spring", duration: 0.7 }} className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-primary text-primary-foreground">
          <Check className="h-10 w-10" />
        </motion.div>
        <h1 className="mt-8 font-display text-4xl font-black tracking-tight md:text-5xl">{t.doneTitle}</h1>
        <p className="mx-auto mt-4 max-w-md text-foreground/70">{t.doneDesc}</p>
        <Button asChild className="mt-8 h-12 rounded-none bg-foreground px-7 text-sm font-bold uppercase tracking-[0.18em] text-background hover:bg-primary">
  ...
  return (
    <>
      <section className="relative overflow-hidden bg-gradient-luxe">
        <div aria-hidden className="pointer-events-none absolute -top-20 right-0 h-[420px] w-[420px] rounded-full bg-primary/10 blur-3xl" />
        <div className="relative mx-auto grid max-w-[1200px] items-center gap-12 px-4 py-20 sm:px-6 md:py-28 lg:grid-cols-12 lg:px-10">
          <div className="text-center lg:col-span-7 lg:text-left">
            <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{page.kicker[lang]}</div>
            <h1 className="mt-5 max-w-4xl font-display text-4xl font-black leading-[1.05] tracking-tight md:text-6xl">
              {page.title[lang]} <span className="bg-gradient-pink bg-clip-text text-transparent">{page.highlight[lang]}</span>
            </h1>
            <p className="mx-auto mt-8 max-w-2xl text-[15px] leading-relaxed text-foreground/75 lg:mx-0">
              {page.description[lang]}
            </p>
            <Button asChild size="lg" className="mt-9 h-12 rounded-none bg-primary px-8 text-sm font-bold uppercase tracking-[0.18em] text-primary-foreground hover:bg-primary/90">
              <a href="#partner-form">{page.primaryCta[lang] || t.start} <ArrowRight className="ml-2 h-4 w-4" /></a>
            </Button>
          </div>
      ...
      <section className="border-t border-border bg-background py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-10">
          <div className="max-w-2xl">
            <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{t.whyKicker}</div>
            <h2 className="mt-4 font-display text-3xl font-black leading-tight tracking-tight md:text-5xl">{t.whyTitle}</h2>
          </div>
          <div className="mt-14 grid gap-6 md:grid-cols-3">
            {WHY_CARDS.map((c) => (
              <article key={c.num} className="group border border-border bg-background p-8 transition hover:border-primary/40 hover:bg-accent/30 md:p-10">
                <div className="flex items-start justify-between">
                  <div className="font-display text-5xl font-black text-foreground/10 transition group-hover:text-primary/30">{c.num}</div>
                  <span className="grid h-12 w-12 place-items-center rounded-full bg-primary/10 text-primary"><c.icon className="h-5 w-5" /></span>
                </div>
                <div className="mt-6 font-display text-xl font-black leading-snug tracking-tight text-foreground">{pick(c.hook)}</div>
                <div className="mt-2 text-[11px] font-bold uppercase tracking-[0.28em] text-primary">{c.title}</div>
                <p className="mt-5 text-[14px] leading-relaxed text-foreground/75">{pick(c.copy)}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-border bg-secondary py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-10">
          <div className="max-w-2xl">
            <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{t.valueKicker}</div>
            <h2 className="mt-4 font-display text-3xl font-black leading-tight tracking-tight md:text-5xl">{t.valueTitle}</h2>
          </div>
          <div className="mt-14 grid gap-px overflow-hidden border border-border bg-border md:grid-cols-2">
            {VALUES.map((v) => (
              <article key={v.num} className="group bg-background p-8 transition hover:bg-accent/30 md:p-12">
                <div className="flex items-start justify-between">
                  <div className="font-display text-5xl font-black text-foreground/10 transition group-hover:text-primary/30">{v.num}</div>
                  <span className="grid h-12 w-12 place-items-center rounded-full bg-primary/10 text-primary"><v.icon className="h-5 w-5" /></span>
                </div>
                <div className="mt-6 text-[11px] font-bold uppercase tracking-[0.28em] text-primary">{v.eng}</div>
                <div className="mt-2 font-display text-2xl font-black tracking-tight">{pick(v.title)}</div>
                <div className="mt-1 text-sm font-semibold text-foreground/55">{pick(v.sub)}</div>
                <p className="mt-5 text-[14px] leading-relaxed text-foreground/75">{pick(v.copy)}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-border bg-background py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-10">
          <div className="max-w-3xl">
            <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{t.brandsKicker}</div>
            <h2 className="mt-4 font-display text-3xl font-black leading-tight tracking-tight md:text-5xl">{t.brandsTitle}</h2>
          </div>
          <div className="mt-14 grid gap-8 md:grid-cols-2">
            {BRAND_SELLING_POINTS.map((brand) => (
              <article key={brand.name} className="grid items-center gap-6 border border-border p-6 md:grid-cols-5 md:p-8">
                <img src={brandProductImages.get(brand.name.trim().toLowerCase()) || brand.fallbackImage} alt={brand.name} className="aspect-square w-full rounded-sm object-cover shadow-sm md:col-span-2" />
                <div className="md:col-span-3">
                  <div className="font-display text-2xl font-black tracking-tight">{brand.name}</div>
                  <p className="mt-4 text-sm leading-relaxed text-foreground/75">{pick(brand.copy)}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="border-t border-border bg-secondary py-24">
        <div className="mx-auto grid max-w-[1200px] items-center gap-10 px-4 sm:px-6 lg:grid-cols-12 lg:px-10">
          <div className="lg:col-span-5">
            <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{t.processKicker}</div>
            <h2 className="mt-4 font-display text-3xl font-black leading-tight tracking-tight md:text-5xl">{t.processTitle}</h2>
            <p className="mt-5 text-sm leading-relaxed text-foreground/65">{t.processDesc}</p>
          </div>
          <div className="space-y-3 lg:col-span-7">
            {t.process.map((item, i) => (
              <div key={item} className="flex items-center gap-5 border border-border bg-background p-5">
                <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-primary text-sm font-black text-primary-foreground">{i + 1}</div>
      ...
      <section className="border-t border-border bg-background py-20">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-10">
          <div className="max-w-2xl">
            <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{t.imagesKicker}</div>
            <h2 className="mt-4 font-display text-3xl font-black tracking-tight md:text-4xl">{t.imagesTitle}</h2>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {t.slots.map((slot, index) => {
              ...
              return (
                <div key={slot} className="group relative overflow-hidden border border-border bg-background">
                  {sample ? (
                    <img src={sample.src} alt={sample.alt} className="aspect-[4/3] w-full object-cover transition duration-500 group-hover:scale-105" />
                  ) : (
                  ...
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent p-5 text-white">
                    ...
                    {sample ? <p className="mt-2 line-clamp-1 text-xs text-white/75">{sample.alt}</p> : <p className="mt-2 text-xs text-white/75">{t.replace}</p>}
                  </div>
                </div>
              );
            })}
      ...
      <section id="partner-form" className="border-t border-border bg-background py-24">
        <div className="mx-auto max-w-[860px] px-4 sm:px-6 lg:px-10">
          <div className="text-center">
            <div className="text-[11px] font-bold uppercase tracking-[0.32em] text-primary">{t.contactKicker}</div>
            <h2 className="mt-3 font-display text-3xl font-black tracking-tight md:text-4xl">{t.contactTitle}</h2>
          ...
          <div className="mt-10">
            <div className="mb-3 flex items-center justify-between text-[11px] font-bold uppercase tracking-[0.22em] text-muted-foreground">
              <span>{t.step} {step + 1} / {steps.length} · {t.formSteps[step]}</span>
          ...
          <form onSubmit={submit} className="mt-8 border border-border bg-background p-6 md:p-10">
                  ...
                  <div>
                    ...
                    <div className="mt-6 grid gap-3">
                      {t.purposes.map((p) => {
                        ...
                        return (
                          <label key={p} className={`flex cursor-pointer items-center gap-4 border p-4 transition ${checked ? "border-primary bg-primary/5" : "border-border hover:border-foreground/30"}`}>
                            <Checkbox checked={checked} onCheckedChange={(v) => {
                              const cur = form.getValues("purposes");
                              form.setValue("purposes", v ? [...cur, p] : cur.filter((x) => x !== p), { shouldValidate: true });
                            }} />
                    ...
                    {form.formState.errors.purposes && <p className="mt-2 text-xs text-destructive">{form.formState.errors.purposes.message as string}</p>}
                  </div>
                )}
                ...
                {step === 1 && (
                  <div className="space-y-5">
                    <Field label={t.companyLabel} error={form.formState.errors.company?.message}>
                      <Input maxLength={200} {...form.register("company")} placeholder={t.companyPh} />
                    </Field>
                    <Field label={t.detailLabel}>
                      <Textarea rows={6} maxLength={1000} {...form.register("message")} placeholder={t.detailPh} />
                    </Field>
                  </div>
                )}
                ...
                {step === 2 && (
                  <div className="space-y-5">
                    ...
                    <Field label={t.email} error={form.formState.errors.email?.message}>
                      <Input type="email" maxLength={200} {...form.register("email")} placeholder="partner@company.com" />
                    </Field>
                    <label className="mt-4 flex cursor-pointer items-start gap-3 border border-border p-4 transition hover:border-foreground/30">
                      <Checkbox checked={!!form.watch("consent")} onCheckedChange={(v) => form.setValue("consent", (v ? true : undefined) as any, { shouldValidate: true })} className="mt-0.5" />
                      <span className="text-sm leading-relaxed text-foreground/75"><span className="font-semibold text-foreground">[{lang === "vi" ? "Đồng ý" : "Agreed"}]</span> {t.consent}</span>
                    </label>
            ...
            <div className="mt-10 flex items-center justify-between border-t border-border pt-6">
              <Button type="button" variant="ghost" disabled={step === 0 || submitting} onClick={() => setStep((s) => Math.max(0, s - 1))} className="rounded-none uppercase tracking-wider">
                <ArrowLeft className="mr-1 h-4 w-4" /> {t.back}
              </Button>
              {step < steps.length - 1 ? (
                <Button type="button" onClick={next} className="h-11 rounded-none bg-foreground px-7 text-sm font-bold uppercase tracking-[0.18em] text-background hover:bg-primary">
                  {t.continue} <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              ) : (
                <Button type="submit" disabled={submitting} className="h-11 rounded-none bg-primary px-7 text-sm font-bold uppercase tracking-[0.18em] text-primary-foreground hover:bg-primary/90">
...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
    <div>
      <Label className="text-[11px] font-bold uppercase tracking-[0.22em] text-foreground/60">{label}</Label>
